...
const mocks = {
  ...
  spotifyConnectCancel: async () => ({ ok: true }),
  spotifyDisconnect: async () => ({ ok: true }),
  githubConnectStart: async () => ({ ok: false, error: 'Not available in dev mode' }),
  githubConnectCancel: async () => ({ ok: true }),
  ...
  calendarConnectGoogleCancel: async () => ({ ok: true }),
